...
def core_path() -> Path:
    ...
    if platform.system() == "Windows":
        ...
        candidates = [root / "engine" / "Deskflow.app" / "Contents" / "MacOS" / "deskflow-core", root / "engine" / "Deskflow" / "Deskflow.app" / "Contents" / "MacOS" / "deskflow-core"]
    else:
...
class DeskflowEngine:
    ...
    def stop_all(self) -> None:
        self.stop_server(); self.stop_client()
    ...
    def stop_server(self) -> None:
        with self._lock:
            proc = self.server_process; self.server_process = None; self._server_connected = False
        self._terminate(proc)

    def stop_client(self) -> None:
        with self._lock:
            proc = self.client_process; self.client_process = None; self._client_connected = False
        self._terminate(proc)

    def start_server(self, server_name: str, client_name: str, direction: str, port: int) -> tuple[bool, str]:
        self.stop_server()
        path = core_path()
        if not path.exists(): return False, f"Deskflow 核心缺失：{path}"
        runtime = app_data_dir() / "runtime"; runtime.mkdir(parents=True, exist_ok=True)
        config_path = runtime / "deskflow-server.conf"
        config_path.write_text(build_server_config(server_name, client_name, direction), "utf-8")
        command = [str(path), "server", "-f", "--debug", "INFO", "--name", server_name, "--address", f":{int(port)}", "-c", str(config_path)]
        ok, message, proc = self._spawn(command, "server")
        if ok:
            with self._lock: self.server_process = proc; self._server_connected = False
    ...
    def start_client(self, server_ip: str, server_port: int, client_name: str) -> tuple[bool, str]:
        ...
        path = core_path()
        if not path.exists(): return False, f"Deskflow 核心缺失：{path}"
        command = [str(path), "client", "-f", "--debug", "INFO", "--name", client_name, f"{server_ip}:{int(server_port)}"]
        ok, message, proc = self._spawn(command, "client")
        if ok:
            with self._lock: self.client_process = proc; self._client_connected = False
    ...
    def _spawn(self, command: list[str], role: str) -> tuple[bool, str, subprocess.Popen | None]:
        ...
        try: proc = subprocess.Popen(command, **kwargs)
        except Exception as exc: return False, f"无法启动键鼠共享核心：{exc}", None
        threading.Thread(target=self._read_output, args=(proc, role), name=f"deskflow-{role}-log", daemon=True).start()
        time.sleep(0.55)
        if proc.poll() is not None:
            log = self.recent_log(role, 8)
    ...
    def _read_output(self, proc: subprocess.Popen, role: str) -> None:
        ...
        for raw in proc.stdout:
            ...
            with self._lock:
                ...
                target.append(line); del target[:-80]
            low = line.lower()
            if role == "server" and ("accepted client connection" in low or "has connected" in low):
                with self._lock: self._server_connected = True
                self.status_cb("connected", "连接成功 · 鼠标可以直接跨屏，键盘会自动跟随")
            elif role == "client" and "connected to server" in low:
                with self._lock: self._client_connected = True
                self.status_cb("remote_connected", "已连接主控电脑 · 正在接受键鼠控制")
            elif "failed to connect" in low or "fatal" in low or "error:" in low:
    ...
    def status(self) -> dict:
        with self._lock:
            return {"server_alive": bool(self.server_process and self.server_process.poll() is None), "client_alive": bool(self.client_process and self.client_process.poll() is None), "server_connected": self._server_connected, "client_connected": self._client_connected, "server_log": self._server_log[-4:], "client_log": self._client_log[-4:]}
