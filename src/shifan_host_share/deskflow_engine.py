...
from .lan_bridge import TcpForwarder, probe_tcp_until
...
def classify_deskflow_log(role: str, line: str) -> tuple[str, str] | None:
    """Translate only protocol-level Deskflow events into product status.

    Deskflow logs ``accepted client connection`` immediately after a raw TCP
    socket is accepted, *before* the Deskflow handshake and screen-name check.
    v0.8 treated that line as a real connection, so our own readiness probes
    could make the host falsely show “已连接”.  A real server connection is the
    later ``client \"NAME\" has connected`` log emitted after Deskflow adopts a
    validated client.
    """
    low = line.lower()
    if role == "server":
        if re.search(r'client\s+"[^"]+"\s+has connected', low):
            return "connected", "连接成功 · 鼠标可以直接跨屏，键盘会自动跟随"
        if re.search(r'client\s+"[^"]+"\s+has disconnected', low):
            return "client_disconnected", "第二台电脑已断开 · 主控仍在等待重新连接"
        if "unrecognised client name" in low or "server refused client with our name" in low:
            return "pair_error", "配对码或屏幕方向与主控端授权不一致"
        # Deliberately ignore "accepted client connection": it is only a TCP
        # accept and is also generated by our own port probes.
        return None

    if role == "client":
        if "connected to server" in low or "ipc: connected to server" in low:
            return "remote_connected", "已连接主控电脑 · 正在接受键鼠控制"
        if "disconnected from server" in low or "server disconnected" in low:
            return "remote_disconnected", "与主控电脑的连接已断开 · Deskflow 正在等待重连"
        if "unrecognised client name" in low or "server refused client with our name" in low:
            return "pair_error", "配对码或屏幕方向与主控端授权不一致"
        if "timed out" in low or "failed to connect" in low or "no route" in low:
            return "network_error", line
        if "fatal" in low or "error:" in low or "critical" in low:
            return "engine_error", line
    return None
...
class DeskflowEngine:
    ...
    def start_server(
        ...
        ready, detail = self._wait_for_listener(proc, BACKEND_HOST, BACKEND_PORT, timeout=10.0, cancel_event=cancel_event)
        if not ready:
            log = self.recent_log("server", 20)
            self._terminate(proc)
            with self._lock:
                self.server_process = None
            return False, detail if detail == "操作已取消" else f"Deskflow 后端没有真正启动监听：{detail}{' | 日志：' + log if log else ''}"
        ...
        if not public_probe.ok:
            ...
            with self._lock:
                self.server_process = None
            return False, public_probe.error if public_probe.error == "操作已取消" else f"TCP {port} 已创建但本机真实连接测试失败：{public_probe.error}"
    ...
    def wait_for_client_connection(
        ...
        while time.monotonic() < deadline:
            ...
            with self._lock:
                ...
                proc = self.client_process
            if connected:
                return True, "Deskflow 协议握手成功"
            if proc is None or proc.poll() is not None:
                return False, f"Deskflow Client 已退出。日志：{self.recent_log('client', 16) or '无'}"
            if cancel_event is not None:
                if cancel_event.wait(0.12):
                    return False, "操作已取消"
            else:
                time.sleep(0.12)
        return False, f"TCP 端口可达，但 Deskflow 在 {timeout:.0f} 秒内没有完成协议握手。日志：{self.recent_log('client', 20) or '无'}"
    ...
    def _read_output(self, proc: subprocess.Popen, role: str) -> None:
        ...
        for raw in proc.stdout:
            ...
            with self._lock:
                ...
                target.append(line)
                del target[:-200]
            event = classify_deskflow_log(role, line)
            if event is None:
                continue
            kind, text = event
            if kind == "connected":
